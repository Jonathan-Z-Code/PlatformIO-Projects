...
///@date    12-29-2024
///@version V1.0 
///@brief   This controller sends a data packet to the reciever car via ESP_NOW.
///         User can control the freqency of packet transmission, as well as the amount of 
///         ADC samples preformed when there is downtime between packet transmissions.

// #defines follow ESP32S pinout
#define POT1_PIN  (36)
#define POT2_PIN  (39)
#define LEFT_POT  (0)
#define RIGHT_POT (1)
...
void averageReadings(void) {
  // micros(); function used to determine how long this function takes to execute
  ...
  for( int i = 0 ; i < NUM_SAMPLES; i++) {
    uint16_t leftSample  = analogRead(POT2_PIN); // left potentiometer in breadboard controller dictates the turning angle
    uint16_t rightSample = analogRead(POT1_PIN); // right potentiometer in breadboard controller dictates the speed of car
  ...
  // micros(); code is used to determine how long this function takes to execute
...
void loop() {
  ...
  if(millis() - lastTime > CONTROLLER_UPDATE_RATE) {
    ...
    userData.turnValue  = avgValue[TURN_POT];
    userData.speedValue = avgValue[SPEED_POT];

    // test implementation to see if reverseMode boolean is transmitted properly (once every 16 transmissions)
    ...
    // debug purposes (print out ADC readings)   
    Serial.println("sent data:");
    Serial.println(userData.turnValue);
    Serial.println(userData.speedValue);
  ...
  // preform averageReadings while the non-blocking timer ticks
  averageReadings();
}